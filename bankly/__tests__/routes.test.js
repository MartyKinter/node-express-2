...
describe("GET /users/[username]", function() {
  ...
  test("should error on bad token signature", async function() {
    const response = await request(app)
      .get("/users/u1")
      .send({ _token: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwidXNlcm5hbWUiOiJpbnZhbGlkLXVzZXIiLCJhZG1pbiI6dHJ1ZSwiaWF0IjoxNTE2MjM5MDIyfQ.HWmZ6GSrC0h7s9IvUjSlNjbILBaoCJLRVroo2bRK8sI" });
    expect(response.statusCode).toBe(401);
  });
...
describe("DELETE /users/[username]", function() {
  ...
  //Test for bug #4
  test("should delete the user from the database", async function() {
    // Make sure the user exists before deletion
    let userExistsBefore = await db.query("SELECT * FROM users WHERE username = 'u1'");
    expect(userExistsBefore.rows.length).toBe(1);
    ...
    let userExistsAfter = await db.query("SELECT * FROM users WHERE username = 'u1'");
    expect(userExistsAfter.rows.length).toBe(0);
  });
});
